...
> 📖 **先看逐模式详解再动手**：[设计模式详解.md](设计模式详解.md) —— 对 GoF 23 种模式逐个讲解：要解决的问题、结构角色、代码落点、适用边界、优缺点和常见误用。
>
> 🔍 **再看模式辨析文档**：[设计模式辨析.md](设计模式辨析.md) —— 专门解答"这两个模式大差不差/分不清"的问题，18 组易混淆模式对逐一拆解（一句话本质 → 判据 → 代码级差异 → 误用场景），含快速定位三问。
...
10. ✅ 已实现：[设计模式辨析.md](设计模式辨析.md) 拆解 18 组易混淆模式对（工厂vs抽象工厂、策略vs状态、装饰器vs代理、观察者vs中介者、命令vs备忘录……），每对含一句话本质、三个判据、代码级差异与误用场景；文末附 5 个对照练习，把一对模式互相改写来加深体会。
